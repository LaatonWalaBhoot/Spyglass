 ...
 */
class WordTokenizer @JvmOverloads constructor(
    private val mConfig: WordTokenizerConfig = WordTokenizerConfig.Builder().build()
) : Tokenizer {
    // --------------------------------------------------
    ...
    override fun isValidMention(text: Spanned, start: Int, end: Int): Boolean {
        ...
        if (!multipleWords && containsExplicitChar) {
            ...
            return Character.isLetterOrDigit(token[1])
    ...
    fun getSearchStartIndex(text: Spanned, cursor: Int): Int {
        ...
        return max(closestToCursor.toDouble(), lineStartIndex.toDouble()).toInt()
    }
    ...
    fun getSearchEndIndex(text: Spanned, cursor: Int): Int {
        ...
        return min(closestAfterCursor.toDouble(), lineEndIndex.toDouble()).toInt()
    }
